...
| 路由 | 典型输入 | 执行方式 |
|---|---|---|
| `qa` | “为什么这个结果是这样”“我不理解叠加定理” | 单个答疑 Agent；独立知识点问题先检索教材，上下文追问直接解释 |
...
- 检索质量门控：题意不完整或知识点无证据时不展示引用；分数门槛按本地混合检索的实际标度校准，并继续要求明确的知识点文本证据；
- 知识图谱内容经过 OCR 噪声过滤，基础定义可使用校核卡片替代不可靠公式 OCR。
- 知识图谱中的“相关章节”只接受章节标题或定义性正文的强证据，普通应用场景不会再被误列为概念出处。
