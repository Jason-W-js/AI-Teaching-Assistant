...
- 教师可上传 PDF、图片或扫描版习题册；`qwen3-vl-plus` 联合 PDF-Extract-Kit 按题提取题号、题干、选项、所属插图、答案与评分点，再分别重排为可打印的无答案题目卷和教师答案卷。
...
2. `qwen3-vl-plus` 逐页返回题型、题目标识、题干/选项/插图/答案 bbox、标准答案和评分点；选择题缺项时会自动二次校对，跨页题干合并时会去除重复复述。
