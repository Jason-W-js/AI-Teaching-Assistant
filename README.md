...
- 教师可上传试卷、课后习题、学习指导书、图片或扫描版习题册；`qwen3-vl-plus` 联合 PDF-Extract-Kit 过滤目录、知识讲解等非题目内容，按题提取题号、共同题干、分层小问、选项、所属插图、参考答案与评分点，并重排为可打印的作业内容和参考答案。
...
2. `qwen3-vl-plus` 逐页过滤非题目内容，返回题型、题目标识、共同题干/分层小问/选项/插图/答案 bbox、参考答案和评分点；选择题缺项时会自动二次校对，跨页题干合并时会去除重复复述。
