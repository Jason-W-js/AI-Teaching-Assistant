...
def _title_slide(prs: Presentation, plan: ParsedLearningPlan) -> None:
    ...
    _text(slide, plan.goal, 0.78, 3.52, 6.55, 1.15, size=18, color="D4EAE6")
...
def _overview_slide(prs: Presentation, plan: ParsedLearningPlan, page: int) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    ...
    _text(slide, plan.goal, 1.02, 2.86, 6.2, 1.05, size=23, color=WHITE, bold=True)
    ...
    for index, principle in enumerate(principles):
        ...
        _text(slide, principle, x + 0.78, 5.36, 2.82, 0.7, size=14, color=INK_SOFT, bold=True)


def _roadmap_slide(prs: Presentation, plan: ParsedLearningPlan, page: int) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    ...
    for row in range(rows):
        ...
        for index, stage in enumerate(row_stages):
            ...
            _text(slide, _shorten(stage.goal, 44), x + 0.2, y + 0.92, width - 0.4, 0.52, size=11, color=INK_SOFT)
...
def _stage_slide(prs: Presentation, stage: PlanStage, index: int, total: int, page: int) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    ...
    _text(slide, stage.goal, 2.42, 2.22, 9.55, 0.66, size=19, color=WHITE, bold=True, valign=MSO_ANCHOR.MIDDLE)
...
def _schedule_slide(prs: Presentation, plan: ParsedLearningPlan, page: int) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _set_background(slide, CREAM)
    ...
    for index, item in enumerate(items):
        ...
        _text(slide, _shorten(item, 78), 2.42, y - 0.09, 9.5, 0.55, size=16, color=INK_SOFT, bold=True)
...
def _metrics_slide(prs: Presentation, plan: ParsedLearningPlan, page: int) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _set_background(slide, TEAL_DARK)
    ...
    for index, metric in enumerate(metrics):
        ...
        _text(slide, _shorten(metric, metric_limit), x + 0.96, y + 0.2, card_w - 1.24, card_h - 0.35, size=14.5, color=WHITE, bold=True, valign=MSO_ANCHOR.MIDDLE)
...
def _closing_slide(prs: Presentation, plan: ParsedLearningPlan, page: int) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _set_background(slide, CREAM)
    _brand(slide)
    _footer(slide, page)
    ...
    for index, action in enumerate(first_actions):
        ...
        _text(slide, _shorten(action, 54), 1.62, y - 0.02, 5.48, 0.58, size=16, color=INK_SOFT, bold=True, valign=MSO_ANCHOR.MIDDLE)
...
def build_learning_plan_ppt(markdown: str, output_path: Path, topic: str = "") -> tuple[ParsedLearningPlan, int]:
    ...
    _title_slide(presentation, plan)
    page = 2
    _overview_slide(presentation, plan, page)
    page += 1
    _roadmap_slide(presentation, plan, page)
    page += 1
    for index, stage in enumerate(plan.stages, start=1):
        _stage_slide(presentation, stage, index, len(plan.stages), page)
        page += 1
    if plan.schedule:
        _schedule_slide(presentation, plan, page)
        page += 1
    _metrics_slide(presentation, plan, page)
    page += 1
    _closing_slide(presentation, plan, page)
...
def learning_plan_ppt_path(root_dir: Path, session_id: str, markdown: str, topic: str = "") -> Path:
    digest = hashlib.sha256(f"v1\0{topic}\0{markdown}".encode("utf-8")).hexdigest()[:20]
